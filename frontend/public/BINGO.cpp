...
void set_Cards(int[][25], int N); //initialises Player's Card with random numbers
...
void remove_Num(int arr[][25], const int num); //removes a specific number (set it to 0) from a player's Card
bool check_Win(int arr[][25]);//checks for win condition and returns true/false
...
void bubbleSort(int scores[], string names[]);//sorts the highscores and their corresponding holders' names
void readScore(string names[], int highScores[]);//reads the highscores and their holders' names into dedicated arrays
void save_Game(fstream& file, int player1[][25], int player2[][25], int turn, string name1, string name2);//saves game after each round into a file
void save_Record(string name, string names[], int highscore[]);//saves or updates a person's highscore
void highScores(string names[], int highscore[]);//displays the top 10 highscores
void Set_History(int gameid, int size, string name1, string name2, int turn);//updates the game's history file after each win
void gameHistory();// reads the game history file containing last 10 game's history and displays them
bool validate_Name(string name, int size);//does the validation of the name
...
int main() {
	...
	bool win, placed,exit;
	char choice = 'z', selection;
	string name1, name2, temp;
	int name1length, name2length;//used late to validate the names
	...
	while (choice != 'e') {  //code runs until user chooses 'e' to exit
		...
		exp(choice); //takes input and handles exception 
		...
		case 'a':
			...
			exp(selection);//takes input and handles exception 
			...
			if (selection == 'n') {    //If option for a new game is selected
				cout << "Enter size : ";
				exp(Size);//takes input and handles exception 
				...
				if (Size > 7) {//set console full screen if size greater than 7 is chosen
					...
					nextLine(5);
					SetConsoleTextAttribute(hConsole, 4 + (3 * 16));//changes colors
				...
				for (int i = 0; i < Size;i++) { //to reset array in case previously a game was exited
			...
			else {//incase user chose to Continue
				...
				if (file.peek() == ' ' || file.peek() == ifstream::traits_type::eof()) {//incase file was made for first time or a game was ended before
				...
				else {//if a record existsin file of a previous game
					file >> temp;
					file.ignore();
					Size = stoi(temp);
					file >> temp;
					...
					getline(file, name1);
					getline(file, name2);
					for (int i = 0; i < Size; i++) { //loop to read player 1 array from file 
						for (int j = 0; j < Size; j++) {
							getline(file, temp, ' ');
							player1[i][j] = stoi(temp);
						}
					}
					for (int i = 0; i < Size; i++) { //loop to read player 2 array from file 
						for (int j = 0; j < Size; j++) {
							getline(file, temp, ' ');
			...
			do {   //game does not end until win becomes true
				...
				exp(choose);//takes input and handles exception 
				cin.ignore();//incase user enters a charachter after number by mistake
				if (choose == -1) {  //if user chooses to exit the current game
					cout << "Game Exited Successfully\n";
					system("pause");
					break; //will break the switch and jump to main menu
				}
				...
				remove_Num(player1, choose);   //calling remove_Num to set chosen number to 0 in both players' cards
				remove_Num(player2, choose);

				if (turn == 1) {       //call check_Win to check win conditions for player, whose turn it is
		...
		case 'b':
			gameHistory();//displays the game history of last 10 games
			break;
		case 'c':
			highScores(names, highscore);//displays the top 10 highscores
			break;
		case 'd':
...
bool already(int arr[][25], const int n) {  //checks if a number exists in a card or not 
	bool flag = false;
	for (int i = 0; i < Size; i++) {     //nested loop to check for a value in array
		for (int j = 0; j < Size; j++) {
			if (arr[i][j] == n) {
...
void set_Cards(int arr[][25], const int N) { //sets a player's card with random numbers
	bool flag = true;
	int num;
...
void display_Card(int arr[][25], const int turn) {  //displays a player's card
	...
	cout << "Game ID : " << GameId << "\t\t\t\t\t\t\t\t(Enter -1 At Any Point To Exit The Game)" << "\n\nPlayer " << turn << " Cards \n\n\n";
...
bool check_Win(int arr[][25]) { //checks for win conditions of a player
	int i = 0, j = 0, count = 0;
...
bool validate_Name(string name, int size) { //does the validation of a name
	for (int i = 0; i < size; i++) {
...
void save_Game(fstream& file, int player1[][25], int player2[][25], int turn, string name1, string name2) { //To save game after each round
	file.open("SaveGame.txt", ios::out);
	file << Size << endl << GameId << endl << turn << endl << name1 << endl << name2 << endl;//game was stored in the same sequence we are reading in
	for (int i = 0; i < Size; i++) {//reads first player's array
		for (int j = 0; j < Size; j++) {
			file << player1[i][j] << " ";
		}
		file << endl;
	}
	for (int i = 0; i < Size; i++) {//reads second player's array
		for (int j = 0; j < Size; j++) {
			file << player2[i][j] << " ";
		}
		file << endl;
...
void bubbleSort(int scores[], string names[]) { //performs bubble sort on the two arrays
	...
	for (int i = 0; i < 100; i++) {
		for (int j = 0; j < 99 - i; j++) {
			if (scores[j] < scores[j + 1]) {//as I want data to be in descending order
				temp = scores[j];
				temporary = names[j];
				scores[j] = scores[j + 1];
				names[j] = names[j + 1];
...
void readScore(string names[], int highscore[]) { //reads highscores from a file into the arrays
	...
	for (int i = 0; i < 100; i++) { //read from file into array the record
		getline(score, names[i], ',');//tokenizing with comma
		score >> highscore[i];
...
void save_Record(string name, string names[], int highscore[]) { //saves the highscores into a file
	...
	bool placed = false;
	int index = 0;
	while (placed == false && index < 100) {  //if persons detail are already in array
		if (name == names[index]) {
	...
	else { //if a new record has to be written
...
void Set_History(int gameid, int size, string name1, string name2, int turn) { //write into file the gamehistory of last 10 games
	fstream history, temporary;
	int lines = 0;
	string temp;
	history.open("GameHistory.txt", ios::in);
	while (!history.eof()) { //loop to read lines in file
		lines++;
		getline(history, temp);//read to move to next line
		if (history.eof()) {
			break;
		}
	}
...
void gameHistory() {//displays the last 10 games' history
	fstream history;
	int i = 1;
	...
	history.open("GameHistory.txt", ios::in);
	while (i <= 10 && getline(history, temp)) {//if getline does not work means there were less than 10 games previously played 
		cout << i << ".\n";
		...
		nextLine(2);
		i++;
		if (i % 3 == 0) { // to properly allow the user to see the history and avoid need of scrolling
